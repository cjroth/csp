...
async fn run(cli: Cli) -> Result<()> {
    ...
    match &cli.cmd {
        Cmd::Init {
            ...
            let vid = vault_id
                .clone()
                .unwrap_or_else(|| format!("vault-{}", &id.node_id().to_hex()[..16]));
            let v = Vault::create(&root, id.clone(), &vid)
                .context("create vault (already a vault here?)")?;
            seed_authorized(&v, authorized_keys)?;
            println!("initialized vault {vid} at {}", root.display());
        ...
        Cmd::Clone {
            ...
            tracing::info!("cloning vault {vault_id} → {}", target.display());
            ...
            tracing::info!("saved peer {url} (config); `ctx watch` will reconnect here");
            let node = Node::new(v);
            let _conn = node.connect(url.clone());
            // Catch-up until the head stabilizes (§6.4) or a timeout.
            ...
            println!(
                "cloned vault {vault_id} into {} (identity {})",
                target.display(),
                idpath.display()
            );
        }
        ...
        Cmd::Watch {
            ...
            if listen.is_some() && v.config.tier == "thin" {
                anyhow::bail!("a thin node must not listen/relay (§7)");
            }
            let mut peers: Vec<String> = v.config.peers.clone();
            peers.extend(peer.clone());
            let context_dir = v.context_dir().to_path_buf();
            tracing::info!(
                "vault {} (tier {}, node {}…) — watching {}",
                v.vault_id(),
                v.config.tier,
                &v.node_id().to_hex()[..12],
                root.display()
            ...
            if let Some(addr) = listen {
                ...
                // Default: wss with a self-signed cert (§10/§17.1) — trust
                // is the ed25519 handshake, not a CA. `--no-tls` → plaintext
                // ws (behind a TLS-terminating proxy, or local/trusted).
                ...
                // One line, on stderr: keeps stdout clean for `--json` /
                // scripting; the e2e harness parses this line for the port.
            ...
            // Establish the watcher first, then the initial reconcile (§5.6)
            // picks up pre-existing edits.
