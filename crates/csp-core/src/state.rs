...
//! record). JSON for transparency; never synced (it is under `.context/`).
...
impl EngineState {
    ...
    pub fn save(&self, context_dir: &Path) -> CspResult<()> {
        let p = Self::path(context_dir);
        // Cross-process exclusive lock: a one-shot (`ctx snapshot`/`restore`)
        // and a running `ctx watch` daemon are two writers of
        // `.context/state`. Serialize the whole load→merge→write so neither
        // clobbers the other (`.context/` is never synced, §11).
        ...
        // Merge-on-save for the *additive* maps (snapshots + known set are
        // monotonic; counters take the max). Materialized hashes are
